...
# Step 0 - Install necessary R packages

Run the below code chunk to check if you have the necessary packages installed.
...
# CHANGE ME - the directory that contains your 'Inputs' folder. Inside 'Inputs', put all of your EoIs, current and past
cEoI_directory <- "~/Documents/GitHub/All_BridgeAI/nonGitHub/demo" 
# CHANGE ME - the file name of the EoI you want to process in this session
cEoI_csv_filename <- "EoI_B8.csv" 
```
...
# Remove the matching rows from the original data frame - Val2 indicates this dataframe has completed validation 2
if (length(matching_indices) != 0){
  cEoI_V12 <- cEoI_Val1[!matching_indices, , drop = FALSE]
} else {cEoI_V12 <- cEoI_Val1}
```
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Size_by_Stage")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = size_stage, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Size_by_Type")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = size_type, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Imapct_by_Forecast")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = impact_forecast, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Solutions")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = solutions, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Sector")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = sector, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Barrier")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = barrier, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
...
out_name <- file.path(paste0(output_dir, '/', in_filename, "_Current")) 
ggplot2::ggsave(paste0(out_name,'.png'), plot = current, bg = "white")
write.csv(cEoI_V12_grouped,paste0(out_name,'.csv'), row.names = FALSE)
