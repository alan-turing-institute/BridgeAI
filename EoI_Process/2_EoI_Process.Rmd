...
message(paste0("Your current directory is: ", current_directory))
...
```{r}
cEoI_file_path <- file.path(cEoI_directory, "Inputs", cEoI_csv_filename)
...
# currently it checks the third version of the EoI batches (versions change when the application form changes)
# the following function basically checks if Demo_B3 column name matches with the input file column names
# when a new form introduced, create a new Demo_B4 file with the updated columns and create a new function demo_match_v4 in the `sub_functions.R` file
demo_match_v3(cEoI_df = cEoI)
```
